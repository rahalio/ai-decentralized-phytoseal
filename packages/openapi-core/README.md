# @phytoseal/openapi-core

OpenAPI contracts for the **DDD codegen starter**: shared `common/` components plus the **identity** sample domain.

**Rule:** Every domain YAML maps to DDD layers (`core` → `services` → `adapters` → `api-server`). After routine YAML edits, regenerate **core only** and handwrite lower layers (see `phytoseal-codegen` skill). Bootstrap of a missing domain may use full multi-layer generate when explicitly requested.
