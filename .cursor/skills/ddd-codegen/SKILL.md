...
Package scope: `@phytoseal` (override via `package_scope` in config when cloning).
...
2. Set `package_scope` (e.g. `@acme`) and rename `@phytoseal/*` packages.
